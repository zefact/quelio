# Quelio (クエリオ)

MySQL / PostgreSQL / SQLite / Valkey 対応の無料デスクトップDBクライアント。
macOS (Apple Silicon) / Windows で動作します。UIは日本語専用です。

**紹介ページ**: https://zefact.github.io/quelio/
**機能詳細（対応バージョン・機能一覧）**: https://zefact.github.io/quelio/support.html

> **ベータ版について**
> Quelioは現在ベータ版 (v0.x) です。不具合が含まれる可能性があり、
> 今後のバージョンで機能や設定ファイルの形式が変わることがあります。
> 重要なデータベースに対して使用する場合は、事前にバックアップを取ることをおすすめします。
> 不具合を見つけた場合は [Issues](../../issues) で報告いただけると助かります。

## 主な機能

- MySQL / PostgreSQL / SQLite / Valkey への接続 (SSH踏み台トンネル対応・鍵認証)
  - MySQLの接続はMariaDB、Valkeyの接続はRedisでもそのまま使えます
  - SSHの代わりに AWS Systems Manager (SSM) のポート転送 / Cloud SQL Auth Proxy も使えます
- 接続先のプロファイル管理 (フォルダ分け・並び替え・アイコン色)
- タブ式セッション、テーブル構造表示 (カラム / インデックス / テーブル情報)
- カラムコメントから「論理名 (日本語名)」を取り出して表示
- テーブル定義の編集 (カラム・インデックスの追加/変更/削除、テーブルの作成・改名・削除・日本語名)
- データタブでの直接編集 (UPDATE / INSERT / DELETE。主キーのあるテーブルのみ)
- SQLエディタ (シンタックスハイライト・入力補完・整形・複数SQL実行・1000行ページング・サーバーサイドソート)
  - 整形の書き方 (カンマの位置・大文字小文字・字下げ・JOINのONの置き方など) は設定で選べます
  - エディタ専用の検索・置換 (⌘/Ctrl+F。大文字小文字・単語単位・正規表現)
- 実行履歴とお気に入り (フォルダ整理・ドラッグでの並べ替え。書きかけSQLは設定で次回起動時に復元)
- EXPLAIN / EXPLAIN ANALYZE の実行計画ビュー (時間バー・ボトルネック強調)
- 実行結果のキャプチャ保存 (SQL＋全行をPNG化)・クエリ結果のCSV出力
- SQLコンソール (実行した全SQLの履歴)
- ヘルプ (実行計画・インデックス・照合順序・ショートカット。接続先のDBに合わせて中身を出し分けます)
- スキーマ一覧・定義書の出力 (CSV / Excel)、スキーマ差分ビューア (2接続の比較)
- 日本語のテストデータ生成 (氏名・住所・電話番号など。外部キーの整合を保ちます)
- ER図の自動生成 (リバース・手動編集・PNG / SVG / Mermaid / PlantUML 出力)
  - DBに繋がずに単独ウィンドウでも使えます。図の上だけのテーブルを置く・カラムの文字色を変える、もできます
- テーブル選択式エクスポート / SQLファイルのインポート (mysqldump / pg_dump / mysql / psql 連携)
- DBのユーザー・権限管理 (一覧・作成・パスワード変更・停止・削除、GRANT / REVOKE。MySQL / PostgreSQLのみ)
- **AI連携 (MCPサーバー)**: お使いのAIクライアント (Claude Code / Claude Desktop / Cursor など) から
  Quelio経由でDBのスキーマ参照・SQL実行ができます。AI側に接続情報は渡しません
- アプリ内アップデート (新しいバージョンの確認とインストール)

## インストール

[Releases](../../releases) から最新版をダウンロードしてください。

| OS | ファイル | 手順 |
|---|---|---|
| macOS | `Quelio_<ver>_aarch64.dmg` | dmgを開いて `Quelio.app` をApplicationsへドラッグ |
| Windows | `Quelio_<ver>_x64-setup.exe` | 実行してウィザードに従う |

Windowsで初回実行時にSmartScreenの警告が出た場合は「詳細情報」→「実行」を選択してください。

エクスポート/インポート機能を使う場合は外部ツールが必要です (設定画面で自動検出されます)。
SQLite・Valkeyは外部ツールなしで利用できます。

```bash
# macOS の例
brew install mysql-client   # mysqldump / mysql
brew install libpq          # pg_dump / psql
```

Windowsは MySQL / PostgreSQL の公式インストーラに含まれる同名のコマンドを使います。
インストール先は設定画面から指定できます。

## セキュリティとプライバシー

- データは **すべてローカルに保存** されます。外部サーバーへの送信・テレメトリは一切ありません
- 接続パスワード/SSHパスフレーズは AES-256-GCM で暗号化し、鍵はOSのキーチェーン
  (macOS: キーチェーン / Windows: 資格情報マネージャー) に保存します。
  この2つが対応環境なので、Linuxではビルドしていません
- SSH踏み台のホスト鍵は TOFU (初回接続時に記録) 方式で検証し、
  鍵が変わった場合は中間者攻撃の可能性があるため接続を拒否します
- TLSを検証しない設定の接続には画面上に注意を出します。
  mysqldump / pg_dump などの外部ツールにも同じTLS設定を引き継ぎます

### AI連携 (MCPサーバー) について

Quelio自身が MCP サーバーになり、AIクライアントから Quelio 経由でDBを触れます。
QuelioにAIを組み込むのではなく、**お使いのAIに Quelio を道具として渡す**形です。

- **AI側に接続情報・パスワード・SSHの設定を一切渡しません**。AIが知るのは「接続名」だけで、
  ホスト名も利用者名も渡りません。AIクライアントの設定ファイルにもDBの情報は書きません
- 待受は**同じPCの中 (127.0.0.1) のみ**で、Quelioを起動している間だけ動きます。
  LANや外部からは接続できません
- 接続にはトークンが必須です (`Authorization: Bearer`)。トークンはいつでも作り直せます
- **既定では1つも公開しません**。接続ごとに「公開しない / 読み取りのみ / 更新も許可」を選びます
- **更新系のSQLは、実行のたびにQuelioの画面で許可を求めます**。
  許可しなければ実行されません (答えないまま2分経つと実行せずに終わります)。
  環境を「本番」にした接続では、そもそも更新を許可できません
- AIが実行したSQLは、すべてSQLコンソールに `[AI]` の印付きで残ります

使いかた: **設定 > AI連携** で有効にし、表示された設定をコピーしてAIクライアントに貼るだけです
(ポートとトークンは埋め込まれているので、書き換えは要りません)。
詳しくは[サポートページ](https://zefact.github.io/quelio/support.html)をご覧ください。

データの保存場所 (macOSの場合):

| 内容 | パス |
|---|---|
| 接続プロファイル・設定 | `~/Library/Application Support/jp.co.zefact.quelio/` |
| AI連携のトークン | 同じフォルダの `mcp_token.json` (所有者だけが読める形で保存) |
| CSV / SQL / キャプチャ出力 | 設定 > 一般の「保存先フォルダ」。未指定ならOSのダウンロードフォルダ |

## 開発 (ソースからビルド)

Rust (stable) と Node.js 20+ が必要です。

```bash
git clone https://github.com/zefact/quelio.git
cd quelio
npm install
npm run tauri dev     # 開発起動
npm run tauri build   # 配布用ビルド
```

リリース手順と、AI連携 (MCP) の設計メモ・疎通確認の手順は
[readme_dev.md](./readme_dev.md) にあります。

## ライセンス

[MIT License](./LICENSE) — © 2026 ZEFACT Co., Ltd. (株式会社ゼファクト)

本ソフトウェアは無保証で提供されます。利用によって生じたいかなる損害についても、
作者および著作権者は責任を負いません (詳細はLICENSEを参照)。
利用しているオープンソースライブラリの一覧は
[THIRD-PARTY-NOTICES.md](./THIRD-PARTY-NOTICES.md) を参照してください。

不具合報告・要望は [Issues](../../issues) へお願いします。
